...
// Raw File:     https://github.com/nrensen/TriggerCookies/raw/master/TriggerCookies/Scripts/Live/AutoCookie.js
...
function LoadTriggerCookies() {
	if (!IsModLoaded('TriggerCookies')) {
		Game.LoadMod(GetModURL() + 'Scripts/Live/TriggerCookies.js');
	}
}
/* Loads the specified Trigger Cookies Mod. */
function LoadMod(name) {
	if (!IsModLoaded(name)) {
		Game.LoadMod(GetModURL() + 'Scripts/Live/' + name + '.js');
	}
}
...
AutoCookie.WriteMenu = function (tab) {
	...
	if (tab == 'Automation') {
		...
		str += '<div class="listing">' +
				...
				'<label style="display: none">This applies for manual resets as well</label>' +
				'</div>';
		...
		var heavenlyChips = Game.prestige['Heavenly chips'];
		...
		var logChips = Math.log(heavenlyChips) / Math.log(10);
		...
		var nextAscend = heavenlyChips * (multiplier - 1);
		...
		str += '<div class="listing"><b>Next reset at :</b>' +
			Helper.Menu.WriteSpacing(8) +
			'<div id="' + iAuto('nextAscend') + '" class="priceoff">' + Beautify(nextAscend) +
...
AutoCookie.ShowAscendPrompt = function () {
	...
	Game.Prompt(str, [['Reset', 'Game.ClosePrompt(); Game.Reset(true);'], ['Disable', 'Game.ClosePrompt(); AutoCookie.Actions["autoascend"].Disable(); AutoCookie.UpdateButtons();']], AutoCookie.UpdateAscendPrompt, 'legacyPrompt');

	//l('promptOption2').style.display = 'none';
	Game.UpdatePrompt();
	AutoCookie.UpdateAscendPrompt();
	l('prompt').style.width = '400px';
	//l('prompt').style.left = '-200px';
...
AutoCookie.UpdateAscendPrompt = function () {
	...
	lAuto('ascendPromptData').innerHTML = '' +
		...
		'<div class="listing"><b>Heavenly chips :</b> ' + Beautify(Game.prestige['Heavenly chips']) + '</div>' +
		'<div class="listing"><b>Ascending now will produce :</b> ' + Beautify(ascendNowToGet) + ' heavenly chip' + ((ascendNowToGet) == 1 ? '' : 's') + '</div>' +
		'<div class="listing"><b>Auto reset will start in :</b> ' + Beautify(60) + ' second' + ((60) == 1 ? '' : 's') + '</div>' +
		//'<div class="listing"><b>Auto reset will start in :</b> ' + Beautify(AutoCookie.AutoAscendTimer) + ' second' + ((AutoCookie.AutoAscendTimer) == 1 ? '' : 's') + '</div>' +
		'';
