...
Overrides.UpdateMenuPrefs = function () {
	...
	//'<div class="listing"><a class="option" ' + Game.clickStr + '="Game.ExportSave();">Export save</a><input type="file" accept=".cookie">Import save</input><label>You can use this to backup your save or to transfer it to another computer</label></div>' +
	//'<a href="img/alteredGrandma.png" download="alteredGrandma.png">Save</a>' +
	//'<button onclick="javascript:document.execCommand(\'SaveAs\',\'1\',\'img/alteredGrandma.png\')">Clickbutton</button>' +
	...
	for (var i in list) {
		...
		if (!Game.Has('Neuromancy')) {
			if (me.bought > 0 && (me.pool == '' || me.pool == 'cookie')) {
				...
				// Added
			else if (me.pool == 'debug') {
				if (me.unlocked > 0 && me.name == 'Neuromancy') {
			...
			// Replaced
			if (me.pool == 'prestige') {
				str2 += '<div ' + Game.clickStr + '="Game.UpgradesById[' + me.id + '].toggle();" class="crate upgrade' + (me.bought > 0 ? ' enabled' : '') + ' heavenly" ' + Game.getTooltip(
				'<div style="min-width:200px;"><div style="float:right;"><span class="price heavenly">' + Beautify(Math.round(me.basePrice)) + '</span></div><small><small style="color:#efa438;">[Heavenly]</small>' + (me.bought > 0 ? ' [Purchased]' : '') + '</small><div class="name">' + me.name + '</div><div class="description">' + me.desc + '</div></div>'
				, 'bottom-right') + ' style="' + (me.icon[2] ? 'background-image:url(' + me.icon[2] + ');' : '') + 'background-position:' + (-me.icon[0] * 48) + 'px ' + (-me.icon[1] * 48) + 'px;"></div>';
				prestigeUpgradesOwned++;
			}
			else {
